...
let {reportSchema} = require('./mongoConfig');
...
function genReport_JSON(data) {
    ...
    fs.writeFile(__dirname + "\\..\\test\\reports\\report.json", JSON.stringify(report, null, 2), 'utf8', function(err) {
        if (err) {
...
function genReport(data) {
    ...
    wb.write('Report.xlsx', function(err, stats) {
        if (err) {
            ...
            fs.writeFile(__dirname + '\\..\\test\\reports\\Report.xlsx', filePath, function(err) {
                if (err) throw err;
            });
        }
    });
