...
function saveReportToDB(data) {
    ...
    return Reports.create(report, function(err, data) {
        if (err) throw err;
        else {
            console.log("Wrote report to DB");
            return data;
        }
    });
}
...
function genReport_JSON(data, tests, passed, program_stats) {
    ...
    let mochaReport = undefined;
    if (os.type() === "WINDOWS_NT") {
        mochaReport = JSON.parse(fs.readFileSync(__dirname + "\\..\\mochawesome-report\\mochawesome.json"));
    } else { 
        mochaReport = JSON.parse(fs.readFileSync(__dirname + "/../mochawesome-report/mochawesome.json"));
    }
    ...
    const report_dir = (
        os.type() === "Windows_NT" 
        ?
            __dirname + "\\..\\test\\reports\\report.json"
        :
            __dirname + "/../test/reports/report.json"
    )

    fs.writeFileSync(report_dir, JSON.stringify(report, null, 2), 'utf8', function(err) {
        if (err) {
            console.log("Error writing json report file: ");
            return console.log(err);
        }

        console.log("Successfully wrote to report.json");
    });
}
    ...
    return wb.write('Report.xlsx', function(err, stats) {
        if (err) {
            ...
            const report_dir = (
                os.type() === "Windows_NT"
                ?
                    __dirname + '\\..\\test\\reports\\Report.xlsx'
                :
                    __dirname + '/../test/reports/Report.xlsx'
            )
            return fs.writeFile(report_dir, filePath, function(err) {
                if (err) throw err;
                return console.log('Wrote to Report xlsx file');
            });
        }
